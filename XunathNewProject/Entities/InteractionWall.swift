...
struct InteractionWall {
    ...
    func create(_ interactionNumber: inout Int,_ firstBlockPosition: inout CGPoint?,_ countBlocks: inout CGFloat) -> SKSpriteNode? {
        ...
        let node = createSKNodeInt(size: size, position: CGPoint(x: xPos, y: yPos), intNumber: interactionNumber)
        interactionNumber += 1
        countBlocks = 0
    ...
    private func createSKNodeInt(size: CGSize, position: CGPoint, intNumber: Int) -> SKSpriteNode {
        let tileNode = SKSpriteNode(color: .blue, size: size)
